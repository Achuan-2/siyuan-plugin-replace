...
class Fe extends b.Plugin {
    ...
    async onload() {
        ...
        (this.isMobile = n === 'mobile' || n === 'browser-mobile'),
            ...
            this.eventBus.on('loaded-protyle', this.replaceMain),
            this.addCommand({
                langKey: 'addreplace',
                hotkey: '',
                langText: 'add replace',
                editorCallback: l => {
...
function Ne(t) {
    // 添加按钮
    if (t.toolbar.element.querySelector('[data-type="replace"]')) return;
    const e = document.createElement('button');
    e.setAttribute('class', 'protyle-toolbar__item b3-tooltips b3-tooltips__n'),
        e.setAttribute('data-type', 'replace'),
